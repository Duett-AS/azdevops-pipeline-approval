...
import { ReleaseRestClient, DeploymentStatus, DeploymentOperationStatus } from "azure-devops-extension-api/Release";
...
export class ReleaseService {
    ...
    async getLinks(releaseApproval: ReleaseApprovalEx): Promise<void> {
    ...
    async fillLinks(approvals: ReleaseApprovalEx[]): Promise<void> {
        const projectService = await SDK.getService<IProjectPageService>(CommonServiceIds.ProjectPageService);
        const project = await projectService.getProject();
        if (!project) return;
        const client: ReleaseRestClient = getClient(ReleaseRestClient);
    ...
    private getReleaseDefinitionsIds(approvals: ReleaseApprovalEx[]) {
